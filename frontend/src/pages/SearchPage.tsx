...
import { MapPin, Zap, Star, RefreshCw, Database } from 'lucide-react';
...
const SearchPage = () => {
  ...
  const handleSeedData = async () => {
    ...
    try {
      ...
      setSeedMessage(res.message || 'Seeding successful!');
      await refetch();
    } catch (err: any) {
      setSeedMessage('Failed to seed stations: ' + (err.message || 'Server error'));
    } finally {
      setIsSeeding(false);
    }
  };
    ...
    <div className="flex-1 flex flex-col md:flex-row h-[calc(100vh-64px)] overflow-hidden">
      ...
      <div className="w-full md:w-[400px] lg:w-[500px] xl:w-[600px] 2xl:w-[800px] h-[50vh] md:h-full flex flex-col bg-white shadow-xl z-10">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {seedMessage && (
            <div className="p-3 bg-emerald-50 text-emerald-800 border border-emerald-200 text-xs rounded-lg text-center">
              {seedMessage}
            </div>
          )}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-10 px-4 space-y-3 bg-amber-50 rounded-xl border border-amber-200">
              <p className="text-sm font-semibold text-amber-800">
                Server is spinning up or unreachable
              </p>
              <p className="text-xs text-amber-600 max-w-sm mx-auto">
                Render free web services sleep after inactivity. Please allow a few seconds for the backend to start up.
              </p>
              <button 
                onClick={() => refetch()}
            ...
            <div className="text-center py-10 px-4 text-slate-500 space-y-4 bg-slate-50 rounded-xl border border-slate-200">
              <div>
                <p className="text-sm font-semibold text-slate-700">No charging stations found</p>
                <p className="text-xs text-slate-500 mt-1">
                  The database may be empty or no stations match your current filter.
                </p>
              </div>
              <button
                onClick={handleSeedData}
                disabled={isSeeding}
                className="inline-flex items-center px-4 py-2 bg-emerald-600 text-white text-xs font-medium rounded-lg hover:bg-emerald-700 transition-colors disabled:opacity-50"
              >
                <Database className="h-4 w-4 mr-2" />
                {isSeeding ? 'Seeding Stations...' : 'Seed Sample Stations'}
              </button>
            </div>
          ) : (
